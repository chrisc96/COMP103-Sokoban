...
public class Sokoban {
    ...
    Stack<ActionRecord> actionStack = ActionRecord.history; // reference to stack stored in ActionRecord class
    private ActionRecord AR1;
    ...
    public void doAction(String direction) {
        ...
        if (squares[nextP.row][nextP.col].hasBox() && squares[nextNextP.row][nextNextP.col].isFree() ) {
            ...
            AR1 = new ActionRecord("push", direction);
            if (isSolved()) {
                UI.println("\n*** YOU WIN! ***\n");
            ...
            AR1 = new ActionRecord("move", direction);
        }
    }
    ...
    public void undo() {
        // checking if stack is empty, if not - don't undo!@!
        if (!actionStack.isEmpty()) {
            ActionRecord A1 = actionStack.pop();
            String dir = oppositeDirection(A1.direction());
            if (A1.action().equalsIgnoreCase("move")) {
                move(dir);
            }

            // if the action of the top stack is push we need to call the pull method
            else if (A1.action().equalsIgnoreCase("push")) {
                pull(dir);
            }
        }
    }
    ...
    public static String oppositeAction(String action) {
        if (action.equalsIgnoreCase("push")) return "pull";
        if (action.equalsIgnoreCase("move")) return "move";
        if (action.equalsIgnoreCase("pull")) return "push"; // For Challenge
        return action;
    }
